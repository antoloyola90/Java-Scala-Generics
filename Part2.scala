...
case class ONode[T <: Ordered[T]](l1: List[OTree[T]]) extends OTree[T] {
  def compare(obj: OTree[T]): Int =
    obj match {
      case ONode(l2: List[OTree[T]]) =>
        if ((l1.head).compare(l2.head) == 0) {
          if (l1.tail.isEmpty && l2.tail.isEmpty) return 0
          else if (l1.tail.isEmpty && !l2.tail.isEmpty) return -1
          else if (!l1.tail.isEmpty && l2.tail.isEmpty) return 1
          else return ONode(l1.tail).compare(ONode(l2.tail))
        } else return (l1.head).compare(l2.head);
      case _ => 1;
    }
}
